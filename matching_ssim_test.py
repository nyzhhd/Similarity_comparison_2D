...
def find_defects_by_comparison_Bolt(original_image, threshold,filename ,num_remove=1):
    have_defect = False
    gk_norm_folder = 'crop_img/registest/'#'gk_mb_20231113'  # 模板库路径
    image_norm_path=gk_norm_folder+filename
    ...
    # print("最大相似度：", mse_max)
    # print("平均相似度：", mse_avg)
    print(filename," 相似度：", mse_sim)

    have_defect = mse_sim <= threshold
    return have_defect, mse_sim
...
if __name__ == "__main__":
    ...
    FN = 0  # 假负例
    threshold = 0.3
    gk_folder ='crop_img\imgtest' # 需要检测图像的文件夹
    ...
    for filename in os.listdir(gk_folder):
        ...
        have_defect, mse_sim = find_defects_by_comparison_Bolt(original_image, threshold,filename)
        total_images += 1
